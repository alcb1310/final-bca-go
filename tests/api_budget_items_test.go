...
func TestApiBudgetItems(t *testing.T) {
	...
	t.Run("should return not exist when non existent supplier", func(t *testing.T) {
		...
		assert.Equal(t, "Partida no encontrada", mapBody["message"])
	})

	t.Run("should be able to update a budget item", func(t *testing.T) {
		req, err := http.NewRequest("GET", testUrl, nil)
		assert.NoError(t, err)
		res := httptest.NewRecorder()
		s.Router.ServeHTTP(res, req)
		...
		assert.Equal(t, 1, len(r))
		for _, v := range r {
			assert.Equal(t, "101", v.(map[string]any)["code"])
